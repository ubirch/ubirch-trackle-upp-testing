package com.ubirch

import com.ubirch.protocol.codec.{MsgPackProtocolDecoder, MsgPackProtocolEncoder}
import com.ubirch.protocol.ProtocolMessage
import com.fasterxml.jackson.databind.node.BinaryNode
import com.ubirch.client.protocol.DefaultProtocolVerifier
import com.ubirch.crypto.GeneratorKeyFactory
import com.ubirch.crypto.utils.Curve
import java.security.MessageDigest
import java.util
import java.util.{Base64, UUID}

import org.bouncycastle.util.encoders.Hex

object Experiment {
  def main(args: Array[String]): Unit = {
    val msgPack: String = ""
    val msgPackByteArray = Hex.decode(msgPack)

    val protocolMessage = MsgPackProtocolDecoder.getDecoder.decode(msgPackByteArray)
    val newProtocolMessage = createNewProtocolMessage(protocolMessage)

    val newMsgpackByteArray = MsgPackProtocolEncoder.getEncoder.encode(newProtocolMessage)
    val newProtocolMessage1 = MsgPackProtocolDecoder.getDecoder.decode(newMsgpackByteArray)

    // Anchoring:: send msg_pack of upp to niomon
    // ---------
    // old_msgpack -> old_pm -> new_pm -> new_msgpack
    // old_msgpack == new_msgpack  because the encoding takes the signed and the
    // signature part

    assert(protocolMessage.getVersion != newProtocolMessage.getVersion)
    assert(protocolMessage.getUUID == newProtocolMessage.getUUID)
    assert(protocolMessage.getPayload.asText() != newProtocolMessage.getPayload.asText())
    assert(protocolMessage.getSignature sameElements newProtocolMessage.getSignature)
    assert(protocolMessage.getChain sameElements newProtocolMessage.getChain)
    assert(protocolMessage.getSigned sameElements newProtocolMessage.getSigned)
    assert(!util.Arrays.equals(msgPackByteArray, newMsgpackByteArray))

    //This test fails because MsgPackProtocolEncoder.getEncoder.encode
    //takes the getSigned and signature as the elements of the byte array, not
    //using the new values.
    assert(newProtocolMessage1.getPayload.asText() == newProtocolMessage.getPayload.asText())

  }

  def createNewProtocolMessage(pm: ProtocolMessage): ProtocolMessage = {
    val newProcotolMessage = new ProtocolMessage()

    val CHAINEDMESSAGE = 35
    newProcotolMessage.setUUID(pm.getUUID)
    newProcotolMessage.setHint(pm.getHint)
    newProcotolMessage.setChain(pm.getChain)
    newProcotolMessage.setSigned(pm.getSigned)
    newProcotolMessage.setVersion(CHAINEDMESSAGE)
    newProcotolMessage.setSignature(pm.getSignature)

    val payloadHash = Hash.sha512.digest(pm.getPayload.asText.getBytes())
    newProcotolMessage.setPayload(BinaryNode.valueOf(Base64.getEncoder.encode(payloadHash)))
    newProcotolMessage
  }
}

object Verifier {

  def execute(pubKeyAsString: String, pm: ProtocolMessage): Boolean = {
    defaultProtocolVerifier(pubKeyAsString).verify(pm.getUUID, pm.getSigned, 0, pm.getSigned.length, pm.getSignature)
  }

  def execute(pubKeyAsString: String, hardwareId: UUID, msgPack: Array[Byte]): Boolean = {
    val dataToVerifyAndSignature = MsgPackProtocolDecoder.getDecoder.getDataToVerifyAndSignature(msgPack)
    defaultProtocolVerifier(pubKeyAsString).verify(hardwareId, dataToVerifyAndSignature(0), 0, dataToVerifyAndSignature(0).length, dataToVerifyAndSignature(1))
  }

  def defaultProtocolVerifier(pubKeyAsString: String): DefaultProtocolVerifier = {
    new DefaultProtocolVerifier((_: UUID) => {
      val decoder = Base64.getDecoder
      import decoder._
      val key = GeneratorKeyFactory.getPubKey(decode(pubKeyAsString), Curve.Ed25519)
      List(key)
    })
  }
}

object Hash {
  val sha512 = MessageDigest.getInstance("SHA-512")
}
